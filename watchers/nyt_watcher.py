...
DEFAULT_NYT_URL = "https://www.nytimes.com/section/business"
DEFAULT_NYT_FEEDS = [
    "https://rss.nytimes.com/services/xml/rss/nyt/Business.xml",
    "https://rss.nytimes.com/services/xml/rss/nyt/World.xml",
]
NYT_SKIP_TITLE_PARTS = ("the morning", "the evening", "here's the latest", "here’s the latest")
...
class NytWatcher(BaseWatcher):
    def __init__(self, source_name: str, target_url: str, interval_seconds: int = WEB_POLL_SECONDS):
        ...
        self.feed_urls = parse_csv_env("NYT_FEEDS", ",".join(DEFAULT_NYT_FEEDS))
        explicit = parse_csv_env("NYT_KEYWORDS", "")
        self.keywords = dedupe_keywords([*explicit, *_keywords_from_trade_symbols()])

    def _fetch_feed_entries(self, feed_url: str) -> list[NytFeedEntry]:
        ...
        for item in root.findall("./channel/item"):
            ...
            if title and url:
                entries.append(NytFeedEntry(url=url, title=title, published_at=published_at, summary=summary, feed_url=feed_url))
        return entries

    def _entry_matches(self, entry: NytFeedEntry) -> bool:
        haystack = f"{entry.title} {entry.summary} {entry.url}".lower()
        if any(part in haystack for part in NYT_SKIP_TITLE_PARTS):
            return False
...
def build_nyt_watchers() -> list[NytWatcher]:
    urls = parse_csv_env("NYT_URLS", DEFAULT_NYT_URL)
    total = len(urls)
    interval = int(os.getenv("NYT_POLL_SECONDS", str(WEB_POLL_SECONDS)))
    return [NytWatcher(make_instance_source_name("nyt", i, total), url, interval) for i, url in enumerate(urls)]
