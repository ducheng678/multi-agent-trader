...
The development default intentionally has no mandatory external service and runs with one application process. When `MARKET_AGENT_POSTGRES_DSN` and `MARKET_AGENT_REDIS_URL` are configured, the container selects the shared PostgreSQL job/event projection and Redis Streams `task.dispatch` transport; workers reclaim pending deliveries and the repository remains the idempotent state authority. Production readiness fails closed if those probes or the Harness/evidence/model/admin authorities are unavailable. Keep the HTTP/API contracts and task names stable while replacing provider adapters.
